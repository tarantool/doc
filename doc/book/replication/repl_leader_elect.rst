...
When the fencing is set to ``soft`` or ``strict``, the leader resigns its leadership if it has less than the
...
Leader *fencing* can be turned off by setting :ref:`election_fencing_mode <repl_leader_elect_config>` configuration parameter to ``off``.

In ``soft`` mode connection is considered dead if the where no responses for
:ref:`4*replication_timeout <cfg_replication-replication_timeout>` both on current leader and followers.

In ``strict`` mode connection is considered dead if the where no responses
for :ref:`2*replication_timeout <cfg_replication-replication_timeout>` on current leader and
:ref:`4*replication_timeout <cfg_replication-replication_timeout>` on followers.
