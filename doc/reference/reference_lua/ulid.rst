...
* under normal workloads you will never hit this limit;
* if an overflow is ever detected, the call to :func:`ulid.new`,
  :func:`ulid.str`, :func:`ulid.bin` (or the module call syntax
  :func:`ulid.__call`) will raise a Lua error;
* applications that generate ULIDs in tight loops can wrap generation
  in ``pcall``/``xpcall`` and, on error, wait for the next millisecond
  or fall back to a different id source.
