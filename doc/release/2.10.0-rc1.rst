...
-   Fixed a crash in JSON update on tuple/space when it had more than one
    ...
    didn’t exist. Example: ``box.tuple.new({1}):update({{'=', 4, 4}, {'=', 3, 3}})``
...
-   Fixed a crash when Tarantool is launched with multiple ``-e`` or
...
-   Now function ``quote()`` will return an argument in case the argument
    is ``DOUBLE``. The same for all other numeric types. For types other
    than numeric, ``STRING`` will be returned (:tarantool-issue:`6239`).
