...
The 3.0 release of Tarantool includes many new features and fixes.
...
-   Added the ``audit_log.audit_extract_key`` option that forces the audit subsystem to log the primary key instead of a full tuple in DML operations.
    This might be useful for reducing an audit log's size in the case of large tuples.
...
In the previous versions, Tarantool already supported a ``varbinary`` type for storing data.
But there was no capability to work with binary data in Lua code.
This required using various workarounds to work with ``varbinary`` fields, for example, using C to work with such data.
...
You can now assign the default values for specific fields when defining a space format.
...
This issue could lead to the following errors:
