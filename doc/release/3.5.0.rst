...
Now the ``replication.failover = supervised`` mode supports the quorum synchronous replication setup in three
availability zones.

A new patchset makes the supervised failover mode support quorum synchronous replication in three availability
zones. The patchset includes several preliminary patches, which make the ``appoint_commit`` logic more safe
